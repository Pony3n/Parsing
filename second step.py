...
#
# all_categories_dict = {}
# reduce = ['product/all', 'product/choice', 'product/pix', 'product/brand', '/cabinet']
...
for category_name, category_href in all_categories.items():
    ...
    with open(f'data_second_step/{count}_{category_name}.html', 'w', encoding='UTF-8') as file:     # Записываем html в файл
        file.write(src)
    ...
    alert_block = soup.find(class_='')

    table_head = soup.find(class_='sticky-enabled').find('tr').find_all('a')   # Находим таблицу, в ней тег tr и в нем все th
